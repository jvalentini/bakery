...
describe('Archetype: Library', () => {
  ...
  beforeAll(() => {
    testDir = createTestDir('library')
    generateProject(createConfig({ archetype: 'library' }), testDir)
  })

  it('should create Library project structure', () => {
    expect(fileExists(testDir, 'tsconfig.json')).toBe(true)
    expect(fileExists(testDir, 'biome.json')).toBe(true)
    expect(fileExists(testDir, 'package.json')).toBe(true)
  })
  ...
  it('should have proper exports configuration', () => {
    const pkg = readJson<{ exports?: Record<string, unknown> }>(testDir, 'package.json')
    expect(pkg.exports).toBeDefined()
  })
})

describe('Archetype: Convex Full-Stack', () => {
  let testDir: string

  beforeAll(() => {
    testDir = createTestDir('convex-fullstack')
    generateProject(
      createConfig({
        archetype: 'convex-full-stack',
        webFramework: 'tanstack-start',
      }),
      testDir,
    )
  ...
  it.skip('should include Convex dependencies', () => {
